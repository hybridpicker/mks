function playtone(x, y){
  ...
  document.querySelector(string + ' img.tone.' + x + '.active').setAttribute('src', '/static/fretboardmedia/yellow_dot_active.svg');
  setTimeout(function () {
    document.querySelector(string + ' img.tone.' + x + '.active').setAttribute('src', '/static/fretboardmedia/yellow_dot.svg');
  }, 300)
  }
}
...
function reset_fretboard(){
  ...
  for (var i=0; i<tone_elements.length; i++) {
    tone_elements[i].setAttribute('src', '/static/fretboardmedia/yellow_dot.svg')
  }
}
